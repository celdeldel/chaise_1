...
class trainer:
    ...
    def train(self):
        ...
        for step in range(config.start_res, self.max_resl+1+5):
...
with open('loss_g.csv', 'w') as csvFile:
    writer = csv.writer(csvFile)
    writer.writerow("nouveau script")
    for r in l1:
        writer.writerow([str(r)])

with open('loss_d.csv', 'w') as csvFile:
    writer = csv.writer(csvFile)
    for r in l2:
        writer.writerow([str(r)])
